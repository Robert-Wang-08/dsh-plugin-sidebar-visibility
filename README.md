...
DSH Web GUI（`dsh web`，默认 `http://127.0.0.1:3080`）增强插件，六项功能共用一个包：
...
1. 本树遮蔽官方树后，仍缺的官方交互：拖拽排序、工作区重命名 / 删除、新建工作区（会话级 重命名 / 分叉 / 归档 与工作区级 新建会话 已补齐）。
...
- 迭代 1（当前）：挂载跑通 + 六功能最小闭环（隐藏 / 折叠 / 新建会话 / 会话行动作 / 收藏 / Provider 挂起）。
- 迭代 2：subagent `allowedModels` 快照剔除；树内搜索框与「仅收藏」对 Host 内容搜索的二次过滤；account key 对齐；补齐官方树交互（拖拽排序、工作区重命名 / 删除）。
